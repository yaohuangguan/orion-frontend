    ...
    <div className="h-full min-h-[80vh] bg-[#4a3728] relative rounded-[2rem] shadow-2xl overflow-hidden border-8 border-[#3e2c22]">
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl border border-slate-200">
            <h3 className="text-xl font-bold text-slate-800 mb-4">
              {t.privateSpace.gallery.pinTitle}
            </h3>

            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-1">
                  {t.privateSpace.gallery.captionLabel}
                </label>
                <input
                  type="text"
                  value={uploadName}
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-1">
                  {t.privateSpace.gallery.dateLabel}
                </label>
                <input
                  type="date"
                  value={uploadDate}
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {isUploading ? (
                    ...
                    t.privateSpace.gallery.pinButton
                  )}
                </button>
              </div>
            </div>
      ...
      <div className="absolute top-0 left-0 w-full h-24 z-10 pointer-events-none">
        <svg className="w-full h-full" preserveAspectRatio="none">
          <path
            d="M0,10 Q150,50 300,10 T600,10 T900,10 T1200,10 T1500,10"
            fill="none"
        ...
        <div
          className="absolute top-8 left-[20%] w-3 h-3 rounded-full bg-green-500 shadow-[0_0_15px_#22c55e] animate-pulse"
          style={{ animationDelay: '0.5s' }}
        ></div>
        <div
          className="absolute top-2 left-[30%] w-3 h-3 rounded-full bg-yellow-500 shadow-[0_0_15px_#eab308] animate-pulse"
          style={{ animationDelay: '1s' }}
        ></div>
        <div
          className="absolute top-8 left-[40%] w-3 h-3 rounded-full bg-blue-500 shadow-[0_0_15px_#3b82f6] animate-pulse"
          style={{ animationDelay: '1.5s' }}
        ></div>
        <div
          className="absolute top-2 left-[50%] w-3 h-3 rounded-full bg-pink-500 shadow-[0_0_15px_#ec4899] animate-pulse"
          style={{ animationDelay: '0.2s' }}
        ></div>
        <div
          className="absolute top-8 left-[60%] w-3 h-3 rounded-full bg-purple-500 shadow-[0_0_15px_#a855f7] animate-pulse"
      ...
      <div className="relative z-20 p-8 flex justify-between items-center bg-black/20 backdrop-blur-sm border-b border-white/10">
        <div>
          ...
          <p className="text-white/60 text-sm font-mono mt-1">{t.privateSpace.gallery.subtitle}</p>
        </div>
      ...
      {isLoading && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-[#4a3728] rounded-[2rem]">
          ...
          <p className="text-amber-100 font-handwriting text-xl animate-pulse">
            {t.privateSpace.gallery.developing}
          </p>
        </div>
      )}
        ...
        >
          <div className="bg-white p-3 pb-4 shadow-xl w-full h-full flex flex-col">
            ...
            <div className="text-center font-handwriting text-[#4a3728] flex-1 overflow-hidden">
              <p className="font-bold text-lg leading-none mb-1 truncate">
                {draggingPhoto.name || draggingPhoto.caption}
              </p>
            </div>
          </div>
        </div>
          ...
          {displayItems.map((photo, idx) => {
            ...
            return (
              ...
              >
                ...
                {isEditing ? (
                  <form
                    onSubmit={(e) => handleSaveUpdate(e, photo._id)}
                  ...
                  <div className="text-center font-handwriting text-[#4a3728]">
                    ...
                    <p className="text-xs opacity-60 font-sans">
                      {photo.createdDate || photo.date
                        ? new Date(photo.createdDate || photo.date || '').toLocaleDateString()
                        : 'Unknown Date'}
                    </p>
                  </div>
                )}
          ...
          {emptySlots.map((_, idx) => (
            ...
            >
              <div className="aspect-[4/5] flex items-center justify-center">
                <span className="text-[#5e4b35] font-handwriting opacity-50 text-xl transform -rotate-12">
                  {t.privateSpace.gallery.reserved}
                </span>
              </div>
