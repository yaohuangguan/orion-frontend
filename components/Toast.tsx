...
export const toast = {
  success: (msg: string) =>
    window.dispatchEvent(new CustomEvent('sys_toast', { detail: { type: 'success', msg } })),
  error: (msg: string) =>
    window.dispatchEvent(new CustomEvent('sys_toast', { detail: { type: 'error', msg } })),
  info: (msg: string) =>
    window.dispatchEvent(new CustomEvent('sys_toast', { detail: { type: 'info', msg } }))
};
      ...
      {toasts.map((t) => (
        ...
        >
          ...
          >
            <i
              className={`fas ${
                t.type === 'success' ? 'fa-check' : t.type === 'error' ? 'fa-bomb' : 'fa-info'
              } text-sm`}
            ></i>
          </div>
