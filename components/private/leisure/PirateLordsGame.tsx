...
export const PirateLordsGame: React.FC = () => {
  ...
  const checkWinCondition = (currentBlocks: GameBlock[]) => {
    const isRedComplete = currentBlocks
      .filter((b) => b.faction === 'RED')
      .every((b) => b.x < 3 && b.y < 3);
    const isBlueComplete = currentBlocks
      .filter((b) => b.faction === 'BLUE')
      .every((b) => b.x > 3 && b.y < 3);
    const isGreenComplete = currentBlocks
      .filter((b) => b.faction === 'GREEN')
      .every((b) => b.x < 3 && b.y > 2);
    const isYellowComplete = currentBlocks
      .filter((b) => b.faction === 'YELLOW')
      .every((b) => b.x > 3 && b.y > 2);
  ...
  const getIcon = (type: UnitType) => {
    switch (type) {
      ...
      case 'CREW':
        return Math.random() > 0.5 ? 'fa-anchor' : 'fa-wine-bottle';
      default:
        return 'fa-circle';
    }
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="relative w-full aspect-[7/6] bg-[#1a1510] rounded-xl overflow-hidden shadow-2xl border-[6px] border-[#3f2e22] select-none">
        <div className="absolute top-0 left-0 right-0 h-10 bg-[#2a1f18] border-b border-[#5e4533] flex items-center justify-between px-4 z-30">
          ...
          <div className="flex gap-4 text-[10px] font-mono text-[#8b735b]">
            <span>
              {t.privateSpace.leisure.pirate.moves}: <span className="text-white">{moves}</span>
            </span>
            <button onClick={initializeBoard} className="hover:text-amber-500 transition-colors">
              <i className="fas fa-redo"></i> {t.privateSpace.leisure.pirate.reset}
            </button>
          </div>
        </div>
        ...
        {isWon && (
          <div className="absolute inset-0 z-50 bg-black/80 flex flex-col items-center justify-center animate-fade-in backdrop-blur-sm">
            ...
            <h2 className="text-3xl text-white font-display font-bold tracking-widest mb-2">
              {t.privateSpace.leisure.pirate.victory}
            </h2>
            <p className="text-[#c2a281] mb-6">{t.privateSpace.leisure.pirate.victoryDesc}</p>
            <button
              onClick={initializeBoard}
              className="px-6 py-2 bg-amber-600 hover:bg-amber-500 text-white font-bold uppercase rounded shadow-lg transition-all"
            >
              {t.privateSpace.leisure.pirate.playAgain}
            </button>
          </div>
        )}
        ...
        {isInitializing && (
          <div className="absolute inset-0 z-50 bg-[#1a1510] flex flex-col items-center justify-center">
            ...
            <p className="text-[#5e4533] font-mono text-xs uppercase tracking-widest">
              Shuffling Deck...
            </p>
          </div>
        )}
        ...
        <div className="absolute inset-0 top-10 flex">
          <div className="w-[42.85%] h-full flex flex-col">
            <div className="h-1/2 bg-red-900/10 border-r border-b border-red-900/20 relative">
              <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
                <i className="fas fa-volcano text-6xl text-red-500"></i>
              </div>
            </div>
            <div className="h-1/2 bg-emerald-900/10 border-r border-t border-emerald-900/20 relative">
              <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
                <i className="fas fa-biohazard text-6xl text-emerald-500"></i>
              </div>
            </div>
          </div>
          ...
          <div className="w-[42.85%] h-full flex flex-col">
            <div className="h-1/2 bg-cyan-900/10 border-l border-b border-cyan-900/20 relative">
              <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
                <i className="fas fa-icicles text-6xl text-cyan-500"></i>
              </div>
            </div>
            <div className="h-1/2 bg-amber-900/10 border-l border-t border-amber-900/20 relative">
              <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
                <i className="fas fa-sun text-6xl text-amber-500"></i>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="absolute inset-0 top-10">
          {blocks.map((block) => {
            ...
            switch (block.faction) {
              case 'RED':
                colorClass = 'bg-[#450a0a] text-red-400';
                borderClass = 'border-red-600';
                glowClass = 'shadow-[0_0_10px_rgba(220,38,38,0.3)]';
                break;
              case 'BLUE':
                colorClass = 'bg-[#082f49] text-cyan-400';
                borderClass = 'border-cyan-600';
                glowClass = 'shadow-[0_0_10px_rgba(8,145,178,0.3)]';
                break;
              case 'GREEN':
                colorClass = 'bg-[#052e16] text-emerald-400';
                borderClass = 'border-emerald-600';
                glowClass = 'shadow-[0_0_10px_rgba(5,150,105,0.3)]';
                break;
              case 'YELLOW':
                colorClass = 'bg-[#451a03] text-amber-400';
                borderClass = 'border-amber-600';
                glowClass = 'shadow-[0_0_10px_rgba(217,119,6,0.3)]';
                break;
              default:
                colorClass = 'bg-[#292524] text-[#57534e]';
                borderClass = 'border-[#44403c]';
            }
            ...
            return (
              ...
              >
                <div
                  className={`w-full h-full border-2 rounded-lg flex items-center justify-center relative overflow-hidden ${colorClass} ${borderClass} ${!isNeutral ? glowClass : 'opacity-90'}`}
                >
                  {isNeutral && (
                  ...
                  {!isNeutral && (
                    <div
                      className={`absolute top-0 right-0 w-3 h-3 rounded-bl-lg ${block.faction === 'RED' ? 'bg-red-500' : block.faction === 'BLUE' ? 'bg-cyan-500' : block.faction === 'GREEN' ? 'bg-emerald-500' : 'bg-amber-500'}`}
                    ></div>
                  )}
                </div>
              </div>
      ...
      <div className="bg-[#2a1f18] rounded-xl p-4 border border-[#5e4533] shadow-lg">
        ...
        >
          <span>
            <i className="fas fa-scroll mr-2"></i> {t.privateSpace.leisure.pirate.rulesTitle}
          </span>
