...
export const FitnessPhotoWall: React.FC<FitnessPhotoWallProps> = ({
  ...
  const groupedPhotos = useMemo(() => {
    ...
    const allRecords = [...wallRecords].sort((a,b) => new Date(b.dateStr || '').getTime() - new Date(a.dateStr || '').getTime());
  ...
  const totalPhotos = useMemo(() => {
      return Object.values(groupedPhotos).reduce((acc: number, curr: { url: string; record: FitnessRecord }[]) => acc + curr.length, 0);
  }, [groupedPhotos]);
          ...
          <div className="flex items-center gap-3">
             ...
             <div className="text-left">
                ...
                <p className="text-[10px] text-rose-400 font-mono">
                   {totalPhotos} Photos in Range
                </p>
             </div>
          </div>
          <div className={`w-8 h-8 rounded-full bg-white border border-rose-100 flex items-center justify-center text-slate-400 transition-transform duration-300 ${isOpen ? 'rotate-180 text-rose-500' : ''}`}>
       ...
       {isOpen && (
          <div className="p-6 bg-slate-50/50 animate-slide-up max-h-[600px] overflow-y-auto custom-scrollbar flex flex-col">
             ...
             <div className="flex justify-between items-center mb-6 bg-white p-2 rounded-xl shadow-sm border border-slate-100">
                <button onClick={onPrevRange} className="px-4 py-2 text-xs font-bold uppercase text-slate-500 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors flex items-center gap-2">
                ...
                <button onClick={onNextRange} className="px-4 py-2 text-xs font-bold uppercase text-slate-500 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors flex items-center gap-2">
             ...
             {Object.keys(groupedPhotos).length === 0 ? (
                ...
                Object.entries(groupedPhotos).map(([monthKey, items]: [string, { url: string; record: FitnessRecord }[]]) => (
                    <div key={monthKey} className="mb-8 last:mb-0">
                        <div className="sticky top-0 bg-slate-50/95 backdrop-blur z-10 py-2 mb-2 border-b border-slate-200">
                            <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">{monthKey}</h4>
                        </div>
                                ...
                                >
                                    ...
                                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-3 pt-8 text-white opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end">
                                        ...
                                        {item.record.body?.weight && (
                                            <span className="text-xs font-bold flex items-center gap-1"><i className="fas fa-weight text-[10px] text-rose-400"></i> {item.record.body.weight}kg</span>
                                        )}
                                    </div>
                                </div>
                            ))}
