...
export const CountDateWidget: React.FC<CountDateWidgetProps> = ({
  ...
  let containerClass =
    'w-full rounded-3xl p-2 text-white shadow-lg relative overflow-hidden group transition-all duration-500 ';
  ...
  if (holidayType === 'CHRISTMAS') {
    ...
    containerClass +=
      'bg-gradient-to-r from-[#7f1d1d] to-[#991b1b] border-2 border-[#b91c1c] shadow-red-900/50';
    ...
    containerClass +=
      'bg-gradient-to-r from-[#7f1d1d] via-[#b91c1c] to-[#9a3412] shadow-amber-900/50 border-2 border-[#b45309]';
    ...
    <div className={containerClass}>
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center justify-between px-2 md:px-6 py-2 gap-4">
          ...
          {hasAccess ? (
            ...
            >
              <span className="text-[10px] uppercase opacity-80 tracking-widest flex items-center gap-2">
                ...
                {holidayLabel && (
                  <span className="bg-white/20 px-1.5 rounded text-[8px] font-bold shadow-sm">
                    {holidayLabel}
                  </span>
                )}
              </span>
              <h3 className="text-lg md:text-xl font-bold font-display text-white transition-all shadow-black/5 drop-shadow-md">
                {showSurprise ? (
                  ...
                  <span>
                    {timeLeft.years}{' '}
                    <span className="text-sm font-normal opacity-80">{t.privateSpace.years}</span>
                    &nbsp;{timeLeft.days}{' '}
                    <span className="text-sm font-normal opacity-80">{t.privateSpace.days}</span>
                  </span>
                )}
              </h3>
            </div>
            ...
            <div className="flex flex-col items-start">
              <span className="text-[10px] uppercase opacity-80 tracking-widest">System Time</span>
        ...
        <div className="flex items-center gap-3">
              ...
              return (
                <button
                  ...
                  className={`relative px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center gap-2 whitespace-nowrap ${
                    isActive ? tabActiveClass : 'text-white/80 hover:bg-white/10 hover:text-white'
                  }`}
                >
                  ...
                  <span className="hidden sm:inline">
                    {
                      t.privateSpace.tabs[
                        tab === 'SECOND_BRAIN'
                          ? 'secondBrain'
                          : (tab.toLowerCase() as keyof typeof t.privateSpace.tabs)
                      ]
                    }
                  </span>
                </button>
          ...
          >
            <i
              className={`fas ${holidayType === 'CHRISTMAS' ? 'fa-tree' : holidayType === 'CNY' ? 'fa-dragon' : 'fa-magic'}`}
            ></i>
          </button>
        </div>
      </div>
